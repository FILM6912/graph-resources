...
interface GpuTotalChartProps {
  ...
  settings: Settings;
  gpuCount: number;
  gpuAvgPercent: number;
  vramUsed: string;
  vramTotal: string;
  tempAvg: number;
}

export const GpuTotalChart = ({
  data,
  settings,
  gpuCount,
  gpuAvgPercent,
  ...
  const gpuY = useMemo(() => data.map(d => d.gpu), [data]);
  const vramY = useMemo(() => data.map(d => d.vram), [data]);
  const tempY = useMemo(() => data.map(d => d.avgTemp), [data]);
    ...
    <div className="chart-container" style={settings.chartHeightMode === 'auto' ? undefined : { height: `${settings.chartHight}px` }}>
      <div className="chart-head-stats">
        <div className="chart-stat-col">
          <span className="status-card-label">GPU ({gpuCount})</span>
          <span className="status-card-value gpu">{pct(gpuAvgPercent, 0)}</span>
          <span className="status-card-sub">Avg usage</span>
        </div>
        <div className="chart-stat-col">
          ...
          <span className="status-card-sub">Total</span>
        </div>
        <div className="chart-stat-col">
          <span className="status-card-label">Temp</span>
          <span className="status-card-value temp">{tempAvg.toFixed(0)}°C</span>
